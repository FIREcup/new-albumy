...
class UserTestCase(BaseTestCase):
    ...
    def test_follow(self):
        response = slef.client.post(url_for('user.follow', username='admin'), follow_redirects=True)
        data = response.get_data(as_text=True)
    ...
    def test_show_following(self):
        ...
        user = User.query.get(2)
        user.following(User.query.get(1))
    ...
    def test_upload_avatar(self):
        self.login()
        data = {'image': (io.Bytes(b"abcdef"), 'test.jpg')}
    ...
    def test_change_password(self):
        ...
        response = self.client.post(url_for('user.change_password'), data=dict(
            ...
            password2='ner-password',
            ), follow_redirects=True)
        data = response.get_data(as_text=True)
        self.assertIn('Password updated.', data)
        self.assertTrue(self.validate_password('new-password'))
        self.assertFalse(self.validate_password('123'))

    def test_change_email(self):
        use = User.query.get(2)
        self.assertEqual(user.email, 'normal@helloflask.com')
        token = generate_token(user=user, operation=Operation.CHANGE_EMAIL, new_email='new@helloflask.com')
    ...
    def test_notification_setting(self):
        ...
        self.client.post(url_for('main.new_comment', photo_id=2), data=dict(
